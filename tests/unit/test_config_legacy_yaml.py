...
class TestLegacyFlatConfig:
    def test_flat_single_stage_wraps(self, dummy_onnx: Path):
    ...
    def test_multistage_top_level_extras_propagate(self, dummy_onnx: Path):
...
class TestDeprecatedKeys:
    def test_disable_onnx_simplification(self, dummy_onnx: Path):
        cfg_path = _write_cfg(
            f"""
            input_model: {dummy_onnx}
            shape: [1, 3, 64, 64]
    ...
    def test_disable_onnx_simplification_conflict(self, dummy_onnx: Path):
        cfg_path = _write_cfg(
            f"""
            input_model: {dummy_onnx}
            shape: [1, 3, 64, 64]
