...
class TestProcessNNArchiveContainer:
    def test_plain_tar(self, work_dir: Path):
    ...
    def test_tar_xz(self, work_dir: Path):
        onnx = standard_dummy_onnx(work_dir / "dummy_model.onnx")
        tar = pack_archive(
    ...
    def test_directory_input(self, work_dir: Path):
    ...
    def test_unsafe_members_are_skipped(self, work_dir: Path):
        onnx = standard_dummy_onnx(work_dir / "dummy_model.onnx")
    ...
    def test_missing_config_raises(self, work_dir: Path):
    ...
    def test_unknown_path_type_raises(self, work_dir: Path):
...
class TestProcessNNArchiveEncodings:
    ENCODINGS = {
...
class TestProcessNNArchiveDaiType:
    """Covers the ``dai_type is not None`` decision tree."""

    def test_rgb_planar_conflicting_reverse_and_interleaved(
        self, work_dir: Path
    ):
    ...
    def test_bgr_interleaved_conflicting_reverse_and_interleaved(
        self, work_dir: Path
    ):
        tar = _pack_single_input(
            work_dir,
    ...
    def test_gray_planar_no_conflicts(self, work_dir: Path):
        tar = _pack_single_input(
            work_dir,
            {"dai_type": "GRAY8p"},
            grayscale=True,
...
class TestProcessNNArchiveDeprecatedFlags:
    """Covers the ``dai_type is None`` (legacy flags) decision tree."""

    def test_reverse_true_interleaved_true(self, work_dir: Path):
        tar = _pack_single_input(
            work_dir,
    ...
    def test_reverse_false_interleaved_false(self, work_dir: Path):
        tar = _pack_single_input(
            work_dir,
    ...
    def test_no_flags_defaults_to_rgb(self, work_dir: Path):
    ...
    def test_grayscale_from_single_channel(self, work_dir: Path):
    ...
    def test_explicit_mean_scale_not_overwritten(self, work_dir: Path):
        tar = _pack_single_input(
            work_dir,
...
class TestProcessNNArchiveRawInput:
    def test_raw_input_keeps_none_encoding(self, work_dir: Path):
        tar = _pack_single_input(
            work_dir,
            {
                "mean": None,
...
class TestProcessNNArchiveHeads:
    def test_postprocessor_path_adds_stage(self, work_dir: Path):
        onnx = standard_dummy_onnx(work_dir / "dummy_model.onnx")
...
class TestPrecisionTable:
    ...
    def test_rvc4_fp16_via_snpe_pairwise_args(self, dummy_onnx: Path):
    ...
    def test_rvc4_fp16_via_snpe_equals_args(self, dummy_onnx: Path):
        config = _config_from_overrides(
            dummy_onnx,
            **{
                "rvc4.quantization_mode": "CUSTOM",
...
class TestConfigToNN:
    def test_output_layout_fallback_on_incompatible_shape(
        self, dummy_onnx: Path
    ):
    ...
    def test_output_default_layout_when_shape_has_zero(self, dummy_onnx: Path):
    ...
    def test_input_default_layout_when_shape_has_zero(self, dummy_onnx: Path):
    ...
    def test_preprocessing_override_applied(self, dummy_onnx: Path):
    ...
    def test_multistage_two_stages_sets_postprocessor(self, dummy_onnx: Path):
        config = Config.get_config(
            None,
            {
                "name": "multi",
    ...
    def test_multistage_without_head_raises(self, dummy_onnx: Path):
        config = Config.get_config(
            None,
            {
                "name": "multi",
    ...
    def test_more_than_two_stages_raises(self, dummy_onnx: Path):
        config = Config.get_config(
            None,
            {
                "name": "multi",
    ...
    def test_raw_input_default_type_without_orig(self, dummy_onnx: Path):
    ...
    def test_raw_input_preprocessing_preserved_from_orig(
        self, dummy_onnx: Path
    ):
...
class TestGetIODtype:
    def test_iop_input_and_output(self, dummy_onnx: Path):
    ...
    def test_iop_name_not_found_raises(self, dummy_onnx: Path):
    ...
    def test_op_output_and_default_input(self, dummy_onnx: Path):
        metadata = get_metadata(dummy_onnx)
    ...
    def test_rvc3_default(self, dummy_onnx: Path):
        metadata = get_metadata(dummy_onnx)
        cfg = RVC3Config()
        assert (
    ...
    def test_rvc4_default(self, dummy_onnx: Path):
        metadata = get_metadata(dummy_onnx)
        cfg = RVC4Config()
        assert (
...
class TestDefaultArchivePreprocessing:
    def test_raw(self):
        inp = InputConfig(
            name="x",
            shape=[1, 3, 64, 64],
    ...
    def test_image_float16_interleaved_with_mean_scale(self):
        inp = InputConfig(
            name="x",
            shape=[1, 3, 64, 64],
            layout="NHWC",
    ...
    def test_image_uint8_planar_without_mean_scale(self):
        inp = InputConfig(
            name="x",
            shape=[1, 3, 64, 64],
            layout="NCHW",
...
class TestArchiveFromModel:
    def test_builds_config_from_onnx(self, dummy_onnx: Path):
...
class TestArchiveInputLookup:
    def test_get_archive_input_found(self, dummy_onnx: Path):
        archive = archive_from_model(dummy_onnx)
        assert get_archive_input(archive, "input1").name == "input1"

    def test_get_archive_input_missing_raises(self, dummy_onnx: Path):
        archive = archive_from_model(dummy_onnx)
        with pytest.raises(ValueError, match="not found"):
            get_archive_input(archive, "nope")

    def test_find_archive_input_none_cfg(self):
        assert find_archive_input(None, "input0") is None

    def test_find_archive_input_found_and_missing(self, dummy_onnx: Path):
        archive = archive_from_model(dummy_onnx)
...
class TestGenerateArchive:
    def _prepare_output(self, dummy_onnx: Path, name: str) -> Path:
        out_dir = OUTPUTS_DIR / name
        out_dir.mkdir(parents=True, exist_ok=True)
        write_json({}, out_dir / "buildinfo.json")
        return out_dir

    def test_single_model_roundtrip(self, dummy_onnx: Path):
        out_dir = self._prepare_output(dummy_onnx, "single")
    ...
    def test_multiple_models_use_stage_name(self, dummy_onnx: Path):
        out_dir = self._prepare_output(dummy_onnx, "multi")
