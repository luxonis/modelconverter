...
class TestModelType:
    ...
    def test_from_suffix(self, suffix: str, expected: ModelType):
        assert ModelType.from_suffix(suffix) == expected

    def test_from_suffix_unsupported(self):
        with pytest.raises(ValueError, match="Unsupported model format"):
            ModelType.from_suffix(".foo")


class TestGetOutputDirName:
    ...
    def test_explicit_and_existing_removed(self):
    ...
    def test_explicit_non_existing_kept(self):
...
class TestGetConfigs:
    def test_single_stage_from_opts(self, dummy_onnx: Path):
    ...
    def test_opts_as_dict(self, dummy_onnx: Path):
        cfg, archive_cfg, main_stage = get_configs(
            Target.RVC4,
            None,
            {"input_model": str(dummy_onnx), "shape": [1, 3, 64, 64]},
    ...
    def test_odd_number_of_opts(self, dummy_onnx: Path):
        with pytest.raises(ValueError, match="Invalid number of overrides"):
            get_configs(Target.RVC4, None, ["input_model"])

    def test_multistage_yolov8_seg_main_stage(self, dummy_onnx: Path):
        cfg_yaml = (
            "name: multi\n"
            "stages:\n"
            "  yolov8n_seg:\n"
        ...
        _cfg, archive_cfg, main_stage = get_configs(
            Target.RVC4, str(cfg_path), []
        )
        assert archive_cfg is None
        assert main_stage == "yolov8n_seg"

    def test_from_nn_archive(self, work_dir: Path):
        ...
        cfg, archive_cfg, main_stage = get_configs(
            Target.RVC4, str(archive), None
        )
        assert archive_cfg is not None
        assert main_stage in cfg.stages

    def test_multistage_without_seg_stage(self, dummy_onnx: Path):
...
class TestExtractPreprocessing:
    def test_multistage_raises(self, dummy_onnx: Path):
        cfg_yaml = (
            "name: multi\n"
            "stages:\n"
            "  a:\n"
    ...
    def test_image_input_rgb_planar(self, dummy_onnx: Path):
    ...
    def test_raw_input_with_mean_scale(self, dummy_onnx: Path):
        cfg = _single_stage_config(
            dummy_onnx,
            encoding="NONE",
            mean_values=[1, 2, 3],
    ...
    def test_raw_input_without_mean_scale(self, dummy_onnx: Path):
        cfg = _single_stage_config(dummy_onnx, encoding="NONE")
        _cfg, preprocessing = extract_preprocessing(cfg)
        assert "input0" not in preprocessing

    def test_image_input_float16_channel_type(self, dummy_onnx: Path):
...
class TestSlugToId:
    ...
    def test_found_public(self, monkeypatch: pytest.MonkeyPatch):
        import modelconverter.cli.utils as cli_utils

        fake = self._FakeRequest([[{"id": "abc123"}]])
        monkeypatch.setattr(cli_utils, "Request", fake)
        assert slug_to_id("my-model", "models") == "abc123"

    def test_http_error_then_found_private(
        ...
        fake = self._FakeRequest([HTTPError("boom"), [{"id": "priv"}]])
        monkeypatch.setattr(cli_utils, "Request", fake)
        assert slug_to_id("my-model", "modelVersions") == "priv"

    def test_not_found_raises(self, monkeypatch: pytest.MonkeyPatch):
        import modelconverter.cli.utils as cli_utils

        fake = self._FakeRequest([[], []])
        monkeypatch.setattr(cli_utils, "Request", fake)
