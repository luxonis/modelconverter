...
class TestGetMetadataOnnx:
    """``.onnx`` dispatch and full introspection of the ONNX backend."""

    def test_multiple_io_various_dtypes(self, work_dir: Path):
    ...
    def test_single_io(self, work_dir: Path):
...
class TestGetMetadataDlc:
    ...
    def test_pipe_table_csv(self, work_dir: Path):
...
class TestGetMetadataDispatch:
    """Suffix dispatch, including vendor-only routes."""

    def test_unsupported_suffix(self, work_dir: Path):
    ...
    def test_vendor_dispatch(
        self, work_dir: Path, suffix: str, vendor_mod: str
    ):
