...
> [!IMPORTANT]
> User-provided `.npy` and `.raw` files are passed to the quantizer without
> color conversion or normalization, including when preprocessing is stored in
> the NN Archive. They must already match the backend's input contract: data
> type, shape, layout, and numeric domain. In particular, RVC4 and Hailo expect
> channel-last data (`NHWC`, or `HWC` for an unbatched sample). Random
> calibration tensors generated by ModelConverter are transformed
