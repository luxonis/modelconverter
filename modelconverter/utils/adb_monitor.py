...
$SYS_MON_APP getPowerStats --q6 cdsp >/data/dsp_read1_full 2>/dev/null

grep '^[[:space:]]*[0-9]*\.[0-9]*[[:space:]]*[0-9]*\.[0-9]*' \
    /data/dsp_read1_full > /data/dsp_read1

sleep $SLEEP_TIME

$SYS_MON_APP getPowerStats --q6 cdsp > /data/dsp_read2_full 2>/dev/null

grep '^[[:space:]]*[0-9]*\.[0-9]*[[:space:]]*[0-9]*\.[0-9]*' \
    /data/dsp_read2_full > /data/dsp_read2
...
class AdbMonitorDSP(_BaseAdbMonitor):
    ...
    This monitor periodically runs `/data/oak_dsp_util.sh` (or an
    ...
    def _prepare_dsp_util_script(self) -> None:
        ...
        ADB."""
        remote_script_path = "/data/oak_dsp_util.sh"
    ...
    def _check_dsp(self) -> bool:
        ...
        try:
            ...
            self.adb_handler.shell(
                "ls -d /data/oak_dsp_util.sh /usr/bin/sysMonApp"
            )
        except Exception:
            logger.warning(
    ...
    def stop(self, full_cleanup: bool = False) -> None:
        ...
        if self.dsp_exists:
            ...
            if full_cleanup:
                files_to_remove.append("/data/oak_dsp_util.sh")
