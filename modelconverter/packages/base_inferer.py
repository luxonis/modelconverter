...
`Inferer` implements the target-independent half of the ``infer``
...
``.npy`` files. Every target -- RVC2, RVC3, RVC4 and Hailo --
...
@dataclass
class Inferer(ABC):
    """Target-independent base for running a converted model.
    ...
    @abstractmethod
    def setup(self) -> None:
        """Initialize the target-specific runtime.
