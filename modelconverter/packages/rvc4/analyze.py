...
class RVC4Analyzer(Analyzer):
    ...
    def _prepare_raw_inputs(
        ...
        self.adb.shell(f"mkdir -p /data/tmp/{self.model_name}/inputs")
        ...
        for i, input_dict in input_matcher.items():
            ...
            for input_name, img_path in input_dict.items():
                ...
                with tempfile.NamedTemporaryFile() as f:
                    ...
                    self.adb.push(
                        f.name,
                        f"/data/tmp/{self.model_name}/inputs/{input_name}_{img_name}.raw",
                    )
                    f.close()

                input_row += f"{input_name}:=/data/tmp/{self.model_name}/inputs/{input_name}_{img_name}.raw "
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            f.close()
        logger.info(
            f"Raw inputs pushed to device at /data/tmp/{self.model_name}/input_list.txt"
        )
        return dlc_matcher
    ...
    def _run_dlc(self, command: str) -> str:
        ...
        try:
            self.adb.push(
                ...
                f"/data/tmp/{self.model_name}/{self.model_name}.dlc",
            )
            self.adb.shell(f"rm -rf /data/tmp/{self.model_name}/output")
            self.adb.shell(f"cd /data/tmp/{self.model_name} && {command}")
            ...
            self.adb.pull(
                f"/data/tmp/{self.model_name}/output",
                f"{target_dir}/output",
            )
            logger.info(
